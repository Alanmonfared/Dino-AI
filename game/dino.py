...
# 50 är det maximala antalet generationer som tillåts för AI-spelet
pygame.init()
...
HEIGHT = 450
WIDTH = 1000
WINDO = pygame.display.set_mode((WIDTH, HEIGHT))
...
class Dinosaur:
    ...
    def draw(self, WINDO, obstacles):
        WINDO.blit(self.img, (self.rect.x, self.rect.y))
        for obstacle in obstacles:
            pygame.draw.line(WINDO, GREEN, (self.rect.x + 54,
                             self.rect.y + 11), obstacle.rect.center, 2)
...
class LargeCactus:
    ...
    def draw(self, WINDO):
        WINDO.blit(self.img[self.type], self.rect)
...
def game_loop(genomes, config):
    ...
    while True:
        ...
        if ground_x <= -GROUND.get_width():
            ground_x = 0
        WINDO.blit(GROUND, (ground_x, ground_y))
        WINDO.blit(GROUND, (ground_x+GROUND.get_rect().width, ground_y))

        # Cloud
        cloud_x -= move_speed/2
        if cloud_x < -100:
            cloud_x = WIDTH
        WINDO.blit(CLOUD, (cloud_x, cloud_y))

        # Dinosaurs
        for dinosaur in dinosaurs:
            dinosaur.move()
            dinosaur.draw(WINDO, obstacles)
        if len(dinosaurs) == 0:
            return
        ...
        for i, dinosaur in enumerate(dinosaurs):
            output = nets[i].activate(
                (dinosaur.rect.y, obstacle.rect.x - dinosaur.rect.x))
        ...
        # stats
        text_1 = FONT.render(
            f'Dinosaurs Alive:  {str(len(dinosaurs))}', True, BLACK)
        text_2 = FONT.render(
            f'Generation:  {population.generation+1}', True, BLACK)
        text_3 = FONT.render(f'Game Speed:  {str(move_speed)}', True, BLACK)
        WINDO.blit(text_1, (600, 50))
        WINDO.blit(text_2, (600, 80))
        WINDO.blit(text_3, (600, 110))
